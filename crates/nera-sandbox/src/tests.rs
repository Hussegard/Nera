...
mod tests {
    ...
    //Proves spawn, pipe capture, and result construction all work together.
    ...
    async fn success_echo_returns_zero_and_captures_stdout() {
        ...
        assert!(result.stdout.as_deref().unwrap_or("").contains("hello"),"stdout must contain 'hello', got: {:?}",result.stdout);
    }

    // "Command ran but reported failure" distinct from "command couldn't start."
    // We don't hardcode the exit code because ls failure codes vary by distro
    ...
    async fn failure_ls_nonexistent_returns_nonzero_and_captures_stderr() {
        ...
        assert_ne!(result.exit_code,Some(0),"ls on a nonexistent path must not exit 0");
        assert!(!result.stderr.as_deref().unwrap_or("").is_empty(),"stderr must contain an error message from ls");
    }
    ...
    async fn timeout_kills_long_running_process() {
        ...
        "sleep 30 with a 1-second timeout must return SandboxError::TimeOut, got: {:?}", result);
    }
    ...
    async fn empty_command_is_rejected() {
        ...
        assert!(matches!(result, Err(SandboxError::FailedToStart)),"empty command must return FailedToStart, got: {:?}",result);
    }
    ...
    async fn whitespace_only_command_is_rejected() {
        ...
        assert!(matches!(result, Err(SandboxError::FailedToStart)),"whitespace only command must return FailedToStart, got: {:?}",result);
    }
    ...
    //and not panic, not an unhandled io::Error surfacing to the caller.
    #[tokio::test]
    async fn nonexistent_program_returns_failed_to_start() {
        ...
        assert!(matches!(result, Err(SandboxError::FailedToStart)),"nonexistent program must return FailedToStart, got: {:?}",result);
    }
    ...
    async fn result_preserves_original_command_string() {
        ...
        assert_eq!(result.command, input,"command field must be the exact original input string, byte-for-byte");
    }

    // possible tests for future implementation
    ...
    //     let result = run_local("sh -c 'echo to_stdout; echo to_stderr >&2'", 5).await.expect("sh should be available and succeed");
    
    //     assert!(result.stdout.as_deref().unwrap_or("").contains("to_stdout"),"stdout must contain 'to_stdout', got: {:?}",result.stdout);
    //     assert!(result.stderr.as_deref().unwrap_or("").contains("to_stderr"),"stderr must contain 'to_stderr', got: {:?}", result.stderr);
    // }
    ...
    async fn silent_command_returns_some_empty_not_none() {
        ...
        assert_eq!(result.stdout,Some(String::new()),"stdout must be Some(\"\") for a silent command, not None");
        assert_eq!(result.stderr,Some(String::new()),"stderr must be Some(\"\") for a silent command, not None");
    }
    ...
    async fn local_process_has_no_sandbox_id() {
        ...
        assert_eq!(result.sandbox_id, None,"local process must never have a sandbox_id");
    }
}